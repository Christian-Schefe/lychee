...
pub fn generate_expression_code(context: &mut CodegenContext, expression: &ResolvedExpression) {
    match &expression.kind {
        ...
        ResolvedExpressionKind::Literal(lit) => {
            match lit {
...
fn generate_assignable_expression_pointer_code(context: &mut CodegenContext, expression: &ResolvedAssignableExpression) {
    match expression {
...
fn generate_assignable_expression_value_code(context: &mut CodegenContext, expression: &ResolvedAssignableExpression, data: &ValueData) {
    match expression {
        ResolvedAssignableExpression::LocalVariable(var_offset) => {
...
fn do_math_op(context: &mut CodegenContext, math_op: &BinaryMathOp, dest_register: &str, source_register: &str) {
    match math_op {
        BinaryMathOp::Add => {
...
fn do_comp_op(context: &mut CodegenContext, comp_op: &BinaryComparisonOp, dest_register: &str, source_register: &str) {
...
fn load_from_value_data(context: &mut CodegenContext, value_data: &ValueData, value_register: &str, address: &str) {
    match value_data.location {
        ValueLocation::Stack => {
            context.movi(value_register, value_data.size as isize);
